...
app.post('/api/sign/:token/submit', async (req, res) => {
  ...
  try {
    ...
    if (pendingSigners.length === 0) {
      ...
      proofPage.drawText('Generated by CML電子署名サービス (Coedo Music Labo)', {
        x: 50,
        y: 40,
        size: 8,
        font: customFont,
      ...
      const mailHtml = `
        <p>関係者 各位</p>
        <p>電子契約システム「CML電子署名サービス」にて、契約書「${contract.title}」の双方の署名が完了し、契約が締結されました。</p>
